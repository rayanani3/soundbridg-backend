...
- **Custom ES256 JWT. 30-day expiry. No refresh token.** Do not introduce Supabase Auth. Do not introduce OAuth, magic links, or passkeys without a cross-repo migration plan. (Algorithm changed from HS256 to ES256 in Week 3 to enable Supabase Realtime auth via the JWKS path; see `docs/migrations/2026-04-jwt-hs256-to-es256.md` for context.)
- **Token payload is `{ id, email, username, sub, role, iat, exp }`.** `req.user.id` is load-bearing in every authenticated handler. `sub` mirrors `id`; `role` is the literal `"authenticated"` required by Supabase Realtime RLS. Never remove, rename, or retype these fields. Adding new claims is tolerable; renaming the existing ones is not.
- **`JWT_PRIVATE_KEY` + `JWT_KID` are the signing material.** A P-256 PEM private key, stored in Render env, with a stable `kid` (UUID) header on every issued token. The matching public key is registered with the Supabase project as a standby signing key and published in the project's JWKS — Supabase verifies our tokens via JWKS and never holds our private key. `jsonwebtoken` derives the public key from the PKCS#8 private internally for local verify, so no separate `JWT_PUBLIC_KEY` env is required. Rotating `JWT_PRIVATE_KEY` requires a coordinated rotation: import a new standby keypair on Supabase, swap Render env, retire the old standby. Either step alone breaks auth — plan the rotation as a single operation. Rotation invalidates every active session.
- **TODO:** annual key rotation. Capture the next rotation date when the migration lands; default cadence is yearly absent a leak event.
- **Legacy HS256 coupling is retired.** The Path A1 framing (Supabase legacy HS256 secret = `JWT_SECRET`) was made unreachable when the Supabase project migrated to ECC signing keys. The current model (Path A2) uses asymmetric verification via JWKS — Supabase trusts our public key without ever holding our private key. `JWT_SECRET` remains in env for 24h post-cutover as a transitional rollback artifact only; it is not on the verify path and must be removed once the soak window closes.
