...
- **Presence RLS expansion (open reads)** — drop the v0 owner-only SELECT policy on `realtime.messages` for `extension = 'presence'`; replace with an open-read policy gated only on `to authenticated`. Unblocks cross-account presence for the bridg-tap airdrop UX (Adam and Rayan must each see the other's bridg light up before either can tap). Write policy unchanged — owner-only. Privacy debt acknowledged; v2 will re-tighten reads to a relationship-gated policy. See `docs/migrations/2026-04-presence-rls-open.md`.
